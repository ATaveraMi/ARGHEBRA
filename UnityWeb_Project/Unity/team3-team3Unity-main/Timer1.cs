...
public class Timer1: MonoBehaviour
{
    ...
    void Update()
    {
        if (!Boton_pausa) // Si nuestro juego no está pausado, seguirá contando el tiempo normal
        {
            ...
            tmp_elapsed.text = "Time: " + elapsedTime.ToString("F1"); // Muestra el tiempo en pantalla con 1 decimal
        }
    }
    ...
    {
        Boton_pausa = false;
        startTime = Time.realtimeSinceStartup - float.Parse(tmp_elapsed.text.Substring(6)); // Esto es para que muestre el tiempo sin contar cuando el juego estaba pausado
    }
}